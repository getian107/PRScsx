...
"""
Markov Chain Monte Carlo (MCMC) sampler for multi-ethnic multi-trait polygenic prediction with continuous shrinkage (CS) priors.
...
def mcmc(a, b, phi, snp_dict, beta_mrg, frq_dict, idx_dict, n, ld_blk, blk_size, n_iter, n_burnin, thin, n_pop, chrom, out_dir, out_name):
    print('... MCMC ...')
    ...
    for itr in range(1,n_iter+1):
        ...
        for pp in range(n_pop):
            ...
            for kk in range(n_blk[pp]):
                if blk_size[pp][kk] == 0:
                    ...
                    beta_tmp = linalg.solve_triangular(dinvt_chol, beta_mrg[pp][idx_blk], trans='T') + sp.sqrt(sigma[pp]/n[pp])*random.randn(len(idx_blk),1)
        ...
        vv = sp.zeros((p_tot,1))
        for pp in range(n_pop):
            vv[idx_dict[pp]] += n[pp]*beta[pp]**2/sigma[pp]

        for jj in range(p_tot):
            while True:
                try:
                    psi[jj] = gigrnd.gigrnd(a-0.5*n_grp[jj], 2.0*delta[jj], vv[jj])
                except:
                    continue
                else:
                    break
    ...
    for pp in range(n_pop):
        beta_est[pp] /= sp.sqrt(2.0*frq_dict[pp]*(1.0-frq_dict[pp]))

    # write posterior effect sizes
    for pp in range(n_pop):
        if phi_updt == True:
            eff_file = out_dir + '/' + '%s_pst_eff_a%d_b%.1f_phiauto_chr%d.txt' % (out_name[pp], a, b, chrom)
        else:
            eff_file = out_dir + '/' + '%s_pst_eff_a%d_b%.1f_phi%1.0e_chr%d.txt' % (out_name[pp], a, b, phi, chrom)
