...
We recommend calculating one polygenic score for each discovery population using population-specific posterior SNP effect size estimates, standardize the resulting scores, and learn a linear combination of the standardized polygenic scores that most accurately predict the trait in the validation dataset. The predictive performance of the method can be assessed in an independent dataset, using the global shrinkage parameter phi (if grid search is used) and weights for the linear combination learnt in the validation dataset.
