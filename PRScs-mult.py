...
"""
PRS-CS-mult: a multi-ethnic multi-trait polygenic prediction method that infers posterior SNP effect sizes under continuous shrinkage (CS) priors
using GWAS summary statistics and external LD reference panels. It is an extension of the polygenic prediction method PRS-CS.
...
Usage:
python PRScs-mult.py --ref_dir=PATH_TO_REFERENCE --bim_prefix=VALIDATION_BIM_PREFIX --sst_file=SUM_STATS_FILE --n_gwas=GWAS_SAMPLE_SIZE
...
def parse_param():
    long_opts_list = ['ref_dir=', 'bim_prefix=', 'sst_file=', 'a=', 'b=', 'phi=', 'n_gwas=', 'pop=',
                      'n_iter=', 'n_burnin=', 'thin=', 'out_dir=', 'out_name=', 'chrom=', 'help']

    param_dict = {'ref_dir': None, 'bim_prefix': None, 'sst_file': None, 'a': 1, 'b': 0.5, 'phi': None, 'n_gwas': None, 'pop': None,
                  'n_iter': 1000, 'n_burnin': 500, 'thin': 5, 'out_dir': None, 'out_name': None, 'chrom': range(1,23)}
    ...
    if len(sys.argv) > 1:
        ...
        for opt, arg in opts:
            ...
            elif opt == "--out_name": param_dict['out_name'] = arg.split(',')
    ...
    if param_dict['ref_dir'] == None:
        ...
        print('* Please specify the population of the GWAS sample --pop\n')
    elif param_dict['out_dir'] == None:
        print('* Please specify the output directory using --out_dir\n')
        sys.exit(2)
    elif param_dict['out_name'] == None:
        print('* Please specify the name of output file using --out_name\n')
        sys.exit(2)
    elif (len(param_dict['sst_file']) != len(param_dict['n_gwas']) or 
          len(param_dict['sst_file']) != len(param_dict['pop']) or
          len(param_dict['sst_file']) != len(param_dict['out_name'])):
        print('* Length of sst_file, n_gwas, pop and out_name does not match\n')
        sys.exit(2)
...
def main():
    ...
    for chrom in param_dict['chrom']:
        ...
        mcmc_gtb.mcmc(param_dict['a'], param_dict['b'], param_dict['phi'], snp_dict, beta_dict, frq_dict, idx_dict, param_dict['n_gwas'], ld_blk, blk_size,
            param_dict['n_iter'], param_dict['n_burnin'], param_dict['thin'], n_pop, int(chrom), param_dict['out_dir'],  param_dict['out_name'])
